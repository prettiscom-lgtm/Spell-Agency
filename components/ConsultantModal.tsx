...
const SlideRenderer: React.FC<{ slide: Slide }> = ({ slide }) => {
  ...
  switch (slide.layout) {
    case 'title':
      return (
        <div className="flex flex-col items-center justify-center h-full text-center p-4 md:p-8 bg-gradient-to-br from-gray-900 to-black text-white">
           <div className="mb-4 w-16 h-1 bg-brand-blue shadow-[0_0_10px_#A0D8EF]"></div>
           <h2 className="text-xl md:text-3xl font-pixel mb-4">{slide.title}</h2>
           <p className="text-xs md:text-sm text-gray-400 tracking-widest uppercase">{slide.subtitle}</p>
        </div>
      );
    case 'big_stat':
      return (
        <div className="flex flex-col items-center justify-center h-full text-center p-4 md:p-6 bg-black">
           <h3 className="text-sm md:text-lg text-gray-400 mb-4">{slide.title}</h3>
           <div className="text-4xl md:text-6xl font-bold text-brand-blue font-pixel mb-2 text-shadow-glow">
             {slide.statValue}
           </div>
           <p className="text-xs md:text-sm text-gray-300 font-mono">{slide.statLabel}</p>
        </div>
      );
    case 'split_image_right':
      return (
        <div className="flex h-full flex-col md:flex-row">
           <div className="w-full md:w-1/2 p-4 md:p-6 flex flex-col justify-center bg-white text-black">
              <h3 className="text-lg md:text-xl font-bold mb-2">{slide.title}</h3>
              <p className="text-[10px] md:text-xs text-gray-600 leading-relaxed">{slide.content}</p>
           </div>
           <div className="w-full md:w-1/2 h-24 md:h-auto bg-gray-800 relative overflow-hidden">
    ...
    case 'quote':
      return (
         <div className="flex flex-col items-center justify-center h-full p-6 md:p-8 bg-brand-blue text-black text-center">
            <blockquote className="text-lg md:text-xl font-bold italic mb-4">"{slide.content}"</blockquote>
    ...
    default:
      return (
        <div className="p-6 h-full flex flex-col bg-white text-black">
          <h3 className="text-lg md:text-xl font-bold mb-4 border-b-2 border-black pb-2">{slide.title}</h3>
...
const ConsultantModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fadeIn p-0 md:p-4">
       <div className={`relative bg-[#050505] w-full h-full md:h-[90vh] md:max-w-[1100px] md:rounded-3xl overflow-hidden border border-white/10 flex flex-col md:flex-row transition-all duration-1000 ${getGlowColor()} ${isMaximized ? 'md:h-screen md:w-screen md:max-w-none md:rounded-none' : ''}`}>
           
           {/* --- LEFT PANEL: Harry's Core (Mobile: Header / Desktop: Sidebar) --- */}
           <div className="w-full md:w-80 bg-[#0a0a0a] border-b md:border-b-0 md:border-r border-white/5 flex flex-row md:flex-col p-4 md:p-6 relative overflow-hidden shrink-0">
               ...
               <div className="flex justify-between items-center mb-0 md:mb-8 relative z-10 w-full md:w-auto">
                   <div className="flex items-center gap-3">
                       <span className="font-pixel text-xl text-white">HARRY<span className="text-xs align-top text-gray-500 ml-1">CORE</span></span>
                       {/* Mobile Status Pulse */}
                       <div className={`md:hidden w-2 h-2 rounded-full ${loading ? 'bg-brand-blue animate-pulse' : 'bg-green-500'}`}></div>
                   </div>
                   <div className="flex gap-2">
                       <div className="w-3 h-3 rounded-full bg-red-500 hover:opacity-80 cursor-pointer" onClick={onClose}></div>
                       <div className="hidden md:block w-3 h-3 rounded-full bg-green-500 hover:opacity-80 cursor-pointer" onClick={() => setIsMaximized(!isMaximized)}></div>
                   </div>
               </div>

               {/* Core Animation - Hidden on Mobile to save space */}
               <div className="hidden md:flex flex-1 flex-col items-center justify-center relative z-10 min-h-[200px]">
               ...
               <div className="hidden md:block bg-white/5 rounded-xl p-4 border border-white/5 backdrop-blur-md relative z-10 mt-auto">
               ...
               <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-10 space-y-6 md:space-y-8">
                   {messages.map((msg) => (
                       <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-slideUp`}>
                           {msg.role === 'assistant' && (
                               <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-white/10 flex items-center justify-center mr-2 md:mr-3 mt-1 text-[10px] font-bold text-white border border-white/20 shrink-0">H</div>
                           )}
                           
                           <div className={`max-w-[85%] rounded-2xl px-4 py-3 md:px-6 md:py-4 ${
                               msg.role === 'user' 
                   ...
                   <div className="relative flex gap-2 md:gap-4">
                       <div className="flex-1 relative">
                           <input 
                                ...
                                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 md:px-5 md:py-4 text-white placeholder-gray-600 focus:outline-none focus:border-brand-blue/50 focus:bg-white/10 transition-all text-sm md:text-base"
                           />
                           <button 
                                onClick={toggleVoice}
                                className={`absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-lg transition-colors ${isListening ? 'text-red-500 bg-red-500/10 animate-pulse' : 'text-gray-500 hover:text-white'}`}
                           >
                       ...
                       <button 
                            ...
                            className="bg-white text-black px-4 md:px-6 rounded-xl font-bold hover:bg-brand-blue transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base"
                       >
                           Send
                       </button>
                   </div>
